...
class GeneticTree:
        ...
        **kwargs,
        ...
        def addPrimitive(func):
            for role in roles:
                ...
                if cls.DEBUG:
                    print(
                        f"importing primitive '{func.__name__}' of type {output_type} for role '{role}'",
                        func,
                    )
            return func
    ...
    def __init__(self, roles, output_type):
        ...
        self.branchingFactor = branchingFactor = max(
            [len(primitive[2]) for primitive in self.primitiveSet]
        )
        self.nodeTags = None
        self.depthLimit = 0
        self.hardLimit = 0
    ...
    def initialize(
        self, depth=1, hardLimit=0, grow=False, leafProb=0.5, full=False, **kwargs
    ):
        if grow:
        ...
        self.depth = math.ceil(
            math.log(max(list(self.nodeTags.keys())), self.branchingFactor)
        )
    ...
    def subtreeMutation(self):
        for i in range(10):
            ...
            mutantDepthLimit = self.hardLimit - math.ceil(
                math.log(target, self.branchingFactor)
            )
            ...
            if self.root.findTag(target, self.branchingFactor).mutate(
                self.primitiveSet, depth
            ):
            ...
            if mutantDepthLimit <= 0:
                ...
                depth = random.randrange(0, mutantDepthLimit)
            self.root.findTag(target, self.branchingFactor).grow(
                self.primitiveSet, depth
            )
    ...
    def subtreeRecombination(self, mate):
        ...
        localTag = random.choice(
            [key for key in self.nodeTags if self.nodeTags[key] in typeOptions]
        )
        mateTag = random.choice(
            [
                key
                for key in mate.nodeTags
                if mate.nodeTags[key] == self.nodeTags[localTag]
            ]
        )
        self.root.assignAtTag(
            localTag,
            mate.root.findTag(mateTag, mate.branchingFactor).copy(),
...
class Node:
    ...
    def getTags(self, branching, index=1):
        ...
        for childIndex in range(len(self.children)):
            tags.update(
                self.children[childIndex].getTags(
                    branching, (index * branching) + childIndex
                )
            )
        return tags
        ...
        if target == index:
            ...
            for childIndex in range(len(self.children)):
                ...
                if tag > target:
                    ...
                    return True
                elif self.children[childIndex].assignAtTag(
                    target, payloadNode, branching, tag
                ):
                    return True
